...
angular.module('bootstrapVariablesEditor.services', []).
  ...
  factory('ap_less',['$http', function($http) {
    ...
    lessEngine.getVariablesToJSON = function ($scope) {
        var tmp, 
            string = "[";
        ...
        for (var i = 0; i < $scope.variables.length; i++ ) {
            ...
            for (var j = 0; j < $scope.variables[i].data.length; j++ ) {
                tmp = (($scope.variables[i].data[j].value).charAt(0) === '#')?'color':'text';
                string += '\n\t\t\t{"key": "'+$scope.variables[i].data[j].key+'", "value": "'+$scope.variables[i].data[j].value+'", "type": "'+tmp+'"}';
                if (j != $scope.variables[i].data.length-1){string += ','}
            }
            string += '\n\t\t]\n\t}';
            if (i != $scope.variables.length-1){string += ','}
        }
